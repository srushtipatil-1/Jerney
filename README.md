...
**Pipeline run** — see the [Actions tab](../../actions) for the live run history on `main`. Real failures currently on record, not staged: the Terraform IaC scan failing on `CKV_TF_1` (see above), and Trivy blocking the frontend image over 34 CRITICAL/HIGH vulnerabilities in its `alpine 3.21.3` base. Also fixed along the way and visible in the commit history: a `bridgecrewio/checkov-action@v12` crash (replaced with a direct, pinned `checkov` invocation) and a `DL3018` Hadolint finding on the backend Dockerfile (explicitly suppressed with a documented reason).
